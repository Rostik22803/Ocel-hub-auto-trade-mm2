...
-- 5. SKIN CHANGER
_G.SelectedKnifeSkin = "Niklis Scythe"
_G.SelectedGunSkin = "Harvester"
_G.SkinChangerActive = false

local SecKnifeSkin = CreateSection(Tabs["SkinChanger"], "Knife Skin Changer")
CreateToggle(SecKnifeSkin, "Enable Skin Changer", false, function(st) _G.SkinChangerActive = st end)

local KnivesList = {"Niklis Scythe", "Icebreaker", "Swirly Blade", "Candycane", "Chroma Elderwood Blade", "Chroma Candleflame", "Chroma Fang", "Chroma Heat", "Corrupt", "Harvester"}
local GunsList = {"Harvester", "Icewing", "Chroma Luger", "Chroma Laser", "Chroma Darkbringer", "Chroma Lightbringer", "Elderwood Revolver", "Swirly Gun"}
...
local function CreateDropdown(parentSec, text, options, defaultVal, callback)
    ...
    Btn.Font = Enum.Font.SourceSansBold
    Btn.Text = "  " .. defaultVal
    Btn.TextColor3 = Color3.fromRGB(220, 35, 45)
    Btn.TextSize = 11
    Btn.TextXAlignment = Enum.TextXAlignment.Left
    ...
    for i, v in ipairs(options) do
        if v == defaultVal then curIdx = i break end
    end
...
CreateDropdown(SecKnifeSkin, "Select Knife Skin", KnivesList, _G.SelectedKnifeSkin, function(choice)
    _G.SelectedKnifeSkin = choice
end)

local SecGunSkin = CreateSection(Tabs["SkinChanger"], "Gun Skin Changer")
CreateDropdown(SecGunSkin, "Select Gun Skin", GunsList, _G.SelectedGunSkin, function(choice)
    _G.SelectedGunSkin = choice
end)
...
-- 3. SKIN CHANGER ENGINE (VISUAL WEAPON & INVENTORY SPOOFER)
local ReplicatedStorage = game:GetService("ReplicatedStorage")
task.spawn(function()
    while true do
        task.wait(0.3)
        if _G.SkinChangerActive then
            -- 1. Подмена моделей оружия в руках и рюкзаке (Tool Models)
            if LocalPlayer.Character then
                pcall(function()
                    ...
                    for _, tool in ipairs(tools) do
                        ...
                        local targetSkinName = nil
                        
                        if lowerName:find("knife") then
                            targetSkinName = _G.SelectedKnifeSkin
                        elseif lowerName:find("gun") or lowerName:find("revolver") then
                            targetSkinName = _G.SelectedGunSkin
                        end
                        
                        if targetSkinName then
                            local weaponDatabase = ReplicatedStorage:FindFirstChild("Weapons") or ReplicatedStorage:FindFirstChild("Items") or ReplicatedStorage
                            local skinModel = weaponDatabase:FindFirstChild(targetSkinName, true)
                            ...
                            if skinModel then
                                ...
                                if handle then
                                    ...
                                    local sourceMesh = skinModel:FindFirstChildOfClass("SpecialMesh", true) or skinModel:FindFirstChildOfClass("MeshPart", true)
                                    if sourceMesh then
                                        local cloneMesh = sourceMesh:Clone()
                                        cloneMesh.Parent = handle
                                    end
                                end
                            end
                        end
            ...
            -- 2. Подмена UI Инвентаря MM2 (Inventory GUI Spoofing)
            pcall(function()
                local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
                if playerGui then
                    for _, gui in ipairs(playerGui:GetChildren()) do
                        if gui:IsA("ScreenGui") and gui.Enabled then
                            for _, itemFrame in ipairs(gui:GetDescendants()) do
                                if itemFrame:IsA("ImageLabel") or itemFrame:IsA("TextLabel") or itemFrame:IsA("Frame") then
                                    local name = itemFrame.Name:lower()
                                    if name:find("item") or name:find("slot") or name:find("weapon") or name:find("knife") or name:find("gun") then
                                        local label = itemFrame:FindFirstChildOfClass("TextLabel") or (itemFrame:IsA("TextLabel") and itemFrame)
                                        if label then
                                            local txt = label.Text:lower()
                                            if txt:find("knife") or txt:find("default knife") then
                                                label.Text = _G.SelectedKnifeSkin
                                            elseif txt:find("gun") or txt:find("default gun") or txt:find("revolver") then
                                                label.Text = _G.SelectedGunSkin
                                            end
                                        end
                                    end
                                end
